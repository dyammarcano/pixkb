...
func TestTagDomain_AppendsDomainSignalForAdjacentTypes(t *testing.T) {
	...
	hits := []Hit{
		...
		{Hit: postgres.Hit{ID: "b", Type: "WebPage"}, Why: []string{SignalLexical}},
	}
	...
	assert.Equal(t, []string{SignalLexical}, hits[1].Why, "WebPage is NOT domain-adjacent to ApiEndpoint (not in the table at all)")
}
